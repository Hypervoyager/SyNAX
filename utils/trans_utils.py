...
class TwoSideNeuron(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.neuron is None:
            self.neuron = torch.zeros_like(x, memory_format=torch.preserve_format)

        # **累加输入**
        self.neuron += x

        # **预计算阈值**
        v_thresholds_p = self.scale_p * (self.gap ** torch.arange(self.times - 1, -1, -1, device=x.device, dtype=x.dtype))
        v_thresholds_n = self.scale_n * (self.gap ** torch.arange(self.times - 1, -1, -1, device=x.device, dtype=x.dtype))

        # **初始化火焰列表和 not_fire**
        fire_list = []
        not_fire = torch.ones_like(x, dtype=torch.bool)

        for v_th_p, v_th_n in zip(v_thresholds_p, v_thresholds_n):
            tmp1 = torch.where(self.neuron >= v_th_p - self.scale_p / 2, v_th_p, 0)
            tmp2 = torch.where(self.neuron <= -v_th_n + self.scale_n / 2, -v_th_n, 0)
