...
from utils.trans_utils import replace_at_by_neuron, replace_nonlinear_by_neuron, replace_testneuron_by_twosideneuron
...
def Summary(args):
    ...
    print("partition:{} \t dir:{}\n".format(args.partition, args.dir))
    print("zo_epsilon:{} \t zero_max:{}, \t zero_min:{}, \t zero_momentum:{}\n".format(args.zo_epsilon, args.zero_max, args.zero_min, args.zero_momentum))
...
# def ann2snn(model, args):
#     snn_model = copy.deepcopy(model)
#     replace_nonlinear_by_neuron(snn_model)
#     replace_at_by_neuron(snn_model)
#     replace_testneuron_by_twosideneuron(snn_model, args)

#     return snn_model


def ann2snn(model, test_data, args):
    snn_model = build_snn_model(args).to(args.device)
    snn_model.load_state_dict(model.state_dict(), strict=False)
    replace_test_by_testneuron(snn_model, 0.99)
    test_stats = evaluate(test_data, snn_model, args.device)
...
def get_params_need_grad(model):
    vec = []
    for param in model.parameters():
        if param.requires_grad:
            vec.append(torch.ones_like(param).cpu().reshape(-1))
        else:
            vec.append(torch.zeros_like(param).cpu().reshape(-1))
    return torch.cat(vec)


def compute_snn_difference(snn_model, snn_old_model):
    # 获取两个模型的参数
    model_params = dict(snn_model.named_parameters())
    old_model_params = dict(snn_old_model.named_parameters())

    # 计算差值（仅对需要梯度的参数）
    delta_params = {}
    for name, param in model_params.items():
        if param.requires_grad:  # 只计算需要梯度的参数
            delta_params[name] = param.data - old_model_params[name].data

    # 归一化处理
    delta_tensor = torch.cat([v.flatten() for v in delta_params.values()])
    norm = torch.norm(delta_tensor)  # 计算 L2 范数
    if norm > 0:
        for name in delta_params:
            delta_params[name] /= norm  # 归一化

    return delta_params, norm

def initialize_device_types(node_num, snn_ratio):
    """
    按顺序分配设备类型（前一部分为 ANN 设备，后一部分为 SNN 设备）。

    Args:
        node_num (int): 设备总数（节点数）。
        snn_ratio (float): SNN 设备的比例（0 到 1）。

    Returns:
        tuple: 包含两个列表，分别是 SNN 设备索引列表和 ANN 设备索引列表。
    """
    # 计算 SNN 和 ANN 设备数量
    num_snn_devices = int(node_num * snn_ratio)
    num_ann_devices = node_num - num_snn_devices

    # 按顺序分配设备类型
    snn_devices = list(range(num_ann_devices, node_num))  # 后一部分为 SNN 设备
    ann_devices = list(range(num_ann_devices))            # 前一部分为 ANN 设备

    return ann_devices, snn_devices