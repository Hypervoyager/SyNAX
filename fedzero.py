...
from utils.trans_utils import replace_nonlinear_by_neuron, replace_at_by_neuron, replace_testneuron_by_twosideneuron
...
for rounds in range(args.R): 
    ...
    for k in range(len(Edge_nodes)):
        ...
        Edge_nodes[k].ann2snn()
        # 需要获取ANN训练前后参数差，可以都转化为SNN后再算参数差，作为扰动。
        # SNN 训练
        for epoch in range(args.E):
            # Edge_nodes[k].model.expand(num_branches = args.capacity_values[k])  # expand branches according to capacity_values[k]
            Train(Edge_nodes[k], epoch, rounds, type = 'SNN')
